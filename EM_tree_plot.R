...
(graph_Biomass_per_plot <- ggplot(p_Biomass_within_site, aes(x = reorder(Plot_code, -Biomass_per_plot),y = Biomass, fill = Country, alpha = 1 ))+
  geom_violin()+ 
  geom_point(aes(alpha = 0.1))+
  # geom_count(inherit.aes = TRUE,)+
  # geom_dotplot(binaxis="y", dotsize = 0.1, stackdir = "center")+
  # geom_jitter(width = 0.2, aes(alpha = 0.4, colour = Country, size = 2))+
  theme_bw()+
  scale_fill_manual(values = colr)+
  scale_color_manual(values = colr)+
  labs(x= "\n Plot code", y = "Mean Biomass (tonnes/ha) \n")+
  theme(axis.text = element_text(size = 12), 
        axis.title = element_text(size = 12), 
        plot.title = element_text(size = 14, hjust = 0.5, face = 'bold'), 
        plot.margin = unit(c(0.5,0.5,0.5,0.5), units = , 'cm'), 
